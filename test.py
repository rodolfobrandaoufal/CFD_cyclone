...
for i in range(1,28,1):
    if i!=11:
        try:
            ...
            vMassica_inlet_p = (((area_inlet/(10**6)))*inlet_df.loc[:,'avg(U.particles (Magnitude))'])*800 #kg/s
            ...
            vMassica_inlet_f = (((area_inlet/(10**6)))*inlet_df.loc[:,'avg(U.air (Magnitude))'])*1.293  #kg/s
            vMassica_overflow_f = ((area_overflow[i-1]/(10**6))*overflow_df.loc[:,'avg(U.air (Magnitude))'])*1.293 #kg/s
            vMassica_underflow_f = ((area_underflow[i-1]/(10**6))*underflow_df.loc[:,'avg(U.air (Magnitude))'])*1.293 #kg/s
            mass_balance_f = vMassica_inlet_f-vMassica_overflow_f-vMassica_underflow_f
